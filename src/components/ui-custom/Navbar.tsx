...
import { Signal, Bell, BookOpen, BarChart3, FolderOpen, Layers, TrendingUp, Lightbulb, HelpCircle, CreditCard, Mail, MapPin, LogIn, LogOut, Briefcase, Search, Bookmark, UserCircle, FileText, Radio, Cpu, Globe, Shield, Award, TrendingUp as TrendIcon, Rocket, ClipboardList, FileCheck, Activity, Globe2 } from 'lucide-react';
  ...
  return (
    <>
      ...
      <nav className="sticky top-0 z-50 h-16 flex items-center justify-between px-4 lg:px-6 bg-canvas/80 backdrop-blur-md border-b border-ink-wash">
        <button onClick={() => handleNavClick('dashboard')} className="flex items-center gap-2 hover:opacity-80 transition-opacity flex-shrink-0">
          <Signal className="w-6 h-6 text-ink-primary" />
          <span className="text-[16px] font-semibold text-ink-primary tracking-tight hidden sm:inline">BuildSignal</span>
        </button>
          ...
          {customerNav.map((item) => (
            <button key={item.id} onClick={() => handleNavClick(item.id)}
              ...
              {item.id === 'alerts' && unreadAlertCount > 0 && (
                <span className="ml-0.5 inline-flex items-center justify-center w-4 h-4 text-[10px] font-bold bg-accent-crimson text-white rounded-full">{unreadAlertCount}</span>
              )}
            </button>
          ))}
        </div>
        ...
        <div className="flex items-center gap-1 flex-shrink-0">
          <div className="md:hidden"><NotificationsPanel /></div>
          ...
          {[
            ...
            { id: 'provider-status', icon: Radio, label: 'Providers' },
            { id: 'county-coverage', icon: Globe, label: 'Counties' },
            { id: 'admin', icon: Shield, label: 'Admin' },
            { id: 'launch-analytics', icon: TrendIcon, label: 'Analytics' },
            { id: 'readiness', icon: Award, label: 'RC1' },
            { id: 'launch-docs', icon: ClipboardList, label: 'Docs' },
            { id: 'gongo', icon: Rocket, label: 'Launch' },
            { id: 'validation', icon: FileCheck, label: 'Validate' },
            { id: 'enterprise-ops', icon: Cpu, label: 'Enterprise' },
            { id: 'patterns', icon: Activity, label: 'Patterns' },
            { id: 'learning', icon: TrendingUp, label: 'Learn' },
            { id: 'intelligence-ops', icon: Cpu, label: 'Ops' },
            { id: 'contact', icon: Mail, label: 'Contact' },
          ].map((item) => (
            <button key={item.id} onClick={() => handleNavClick(item.id)}
              className={`hidden lg:flex items-center gap-1 px-2.5 py-1.5 rounded-lg text-[11px] font-medium transition-colors ${
                currentPage === item.id ? 'text-accent-indigo bg-accent-indigo/10' : 'text-ink-tertiary hover:text-ink-primary hover:bg-surface'
              }`}>
              <item.icon className="w-3.5 h-3.5" />{item.label}
            </button>
          ))}
          ...
          <button onClick={() => handleNavClick('settings')} className="p-2 rounded-full hover:bg-surface transition-colors" aria-label="Settings">
          ...
          {user ? (
            <button onClick={() => logout()} className="hidden lg:flex items-center gap-1 px-2.5 py-1.5 rounded-lg text-[11px] font-medium text-ink-tertiary hover:text-accent-crimson hover:bg-accent-crimson/10 transition-colors" title="Sign out">
              <LogOut className="w-3.5 h-3.5" />
            </button>
          ) : (
            <button onClick={() => handleNavClick('login')} className="hidden lg:flex items-center gap-1 px-3 py-1.5 rounded-lg text-[11px] font-medium bg-accent-indigo text-white hover:bg-accent-indigo/90 transition-colors">
      ...
      <div className="md:hidden fixed bottom-0 left-0 right-0 z-50 bg-canvas/90 backdrop-blur-md border-t border-ink-wash flex items-center justify-around safe-area-pb">
        {[
          ...
          <button key={item.id} onClick={() => setCurrentPage(item.id)}
            ...
            }`} aria-label={item.label} aria-current={currentPage === item.id ? 'page' : undefined}>
