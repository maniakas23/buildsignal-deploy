...
export const validationRouter = createRouter({
  list: publicQuery.input(z.object({
    ...
    const db = (ctx as any).db; if (!db) return { records: [], total: 0, byStatus: {} };
    const conditions = [];
  ...
  stats: publicQuery.query(async ({ ctx }) => {
    ...
    if (!db) return { totalRecords:0, pending:0, passed:0, failed:0, inReview:0, passRate:0, avgConfidence:0, todayProcessed:0, errorsByType:[] };
  ...
  bulkValidate: publicQuery.input(z.object({ recordIds: z.array(z.number()), status: z.enum(["passed","failed"]) })).mutation(async ({ ctx, input }) => {
    const db = (ctx as any).db; if (!db) return { success: false, processed: 0 };
    let processed = 0;
