...
export const enrichmentRouter = createRouter({
  ...
  summary: publicQuery.input(z.object({ eventId: z.number() })).query(async ({ ctx, input }) => {
    const db = (ctx as any).db; if (!db) return { eventId: input.eventId, enrichments: [], types: [], avgConfidence: 0 };
  ...
  stats: publicQuery.query(async ({ ctx }) => {
    ...
    if (!db) return { totalEnrichments:0, byType:[], avgConfidence:0, todayProcessed:0, topSources:[] };
